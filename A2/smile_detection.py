...
def data_preprocessing_A2(dataset):
    """
    Our custom preprocessing class, for preparing data for Task A2:
    ...
    def extract_2D_features(images, labels):
        ...
        X_train_img = X_train_img.to_numpy().reshape((X_train_img.shape[0], 100, 100, 1))
        X_dev_img = X_dev_img.to_numpy().reshape((X_dev_img.shape[0], 100, 100, 1))
        X_test_img = X_test_img.to_numpy().reshape((X_test_img.shape[0], 100, 100, 1))
    ...
    def main(dataset):
        ...
        X_train_img, X_dev_img, X_test_img, y_train_img, y_dev_img, y_test_img = extract_2D_features(reshaped_images, labels)
        return X_train_img, X_dev_img, X_test_img, y_train_img, y_dev_img, y_test_img

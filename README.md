...
* In *.py* files, only final best performing model's evaluation code is included with the best preprocessing technique for each task, besides accuracy reports (including metrics and confusion matrixes) for each train, validation and test data. In this file we load the pretrained models, instead of training them from the beginning. If you want to run the process of training, follow the instructions of [Code Usage](#Code-Usage).
